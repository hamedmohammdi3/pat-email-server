...
import com.fanap.fanrp.pat.patemailserver.ClasspathUtils;
...
import com.fanap.fanrp.pat.patemailserver.odata.ODataRequest;
...
public abstract class DirbyDbDao implements DbDao {
	...
	protected void init(String jdbcUrl, boolean create) throws SQLException {
		...
		if (create) {
			...
			String schemaFileName = new File(DirbyDbDao.class.getResource("/patEmail/db/schema.sql").getPath()).getAbsolutePath();
			Statement statement = null;
			try {
				InputStream inputStream = new FileInputStream(schemaFileName);
				in = new SQLStatementReader(new InputStreamReader(inputStream));
	...
	public int selectDbVersion() throws SQLException {
		...
		try {
			statement = db.prepareStatement("SELECT db_schema_version FROM patMail");
	...
	public void updateDbVersion(int version) throws SQLException {
		/*PreparedStatement statement = null;
		try {
			statement = db.prepareStatement("UPDATE patMail SET db_schema_version = ?");
	...
	public void insertDbVersion(int version) throws SQLException {
		/*PreparedStatement statement = null;
		try {
			statement = db.prepareStatement("INSERT INTO patMail (db_schema_version) VALUES (?)");
			statement.setInt(1, version);
			statement.execute();
		} finally {
			closeStatements(statement);
	...
	public void insertInboxEmail(Email email) throws SQLException {
		...
		try {
			...
			insertInbox.setTimestamp(2, new Timestamp(System.currentTimeMillis()));
			for (EmailAddress recipient : email.recipients) {
	...
	public void insertOutboxEmail(Email email) throws SQLException {
		...
		try {
			...
			insertOutbox.setTimestamp(2, new Timestamp(System.currentTimeMillis()));
...
*/
	...
	@Override
	public List<POPEmail> selectEmailsForPOP(User user) throws Exception {

		String filter = "substringof('" + user.username + "', tolower(email))";
		List<POPEmail> emails = ODataRequest.loadInboxByPerson(filter);
	...
	public User selectUser(String username) throws SQLException {
		User user = new User();
		user.id = 1;
		user.username = "test@pat.ir";
		user.password = "123456";
		user.fullName = "test@pat.ir";
		return user;
