		...
		public void run() {
			try {
				...
				while ((request = serverSocket.nextRequest()) != null) {
					...
					System.out.println("POP3 MSG: " + cmd + " | " + params);
					...
					if ("APOP".equals(cmd)) {
						...
						if (UserManagement.findToken(username)) {
						...
						try {
							User user = UserManagement.getTokenFromEngine(username);
							if (user == null) {
								serverSocket.sendError("Unknown user " + username + ".");
								continue;
							}
						...
						try {
							...
							String data = EmailConvertor.resultSet.get(popId);
							POPEmail email = EmailConvertor.resultSetEmail.get(popId);
						...
						serverSocket.sendSuccess("Ok, Capability list follows\n USER.");
					} else {
